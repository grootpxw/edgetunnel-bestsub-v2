...
import (
	...
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/app"
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/clash"
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/config"
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/preflight"
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/probe"
	"github.com/grootpxw/edgetunnel-bestsub-v2/internal/worker"
)
...
func (s *Server) handleConfigUpdate(w http.ResponseWriter, r *http.Request) {
	...
	// 更新配置项
	if countries, ok := req["countries"].([]interface{}); ok {
		strCountries := make([]string, 0, len(countries))
		for _, c := range countries {
			if str, ok := c.(string); ok {
				strCountries = append(strCountries, str)
			}
		}
		s.cfg.Probe.Countries = strCountries
	}

	// 更新 worker.base_url
	if baseURL, ok := req["worker.base_url"].(string); ok && baseURL != "" {
		s.cfg.Worker.BaseURL = baseURL
	}

	// 更新 worker.password
	if password, ok := req["worker.password"].(string); ok && password != "" {
		s.cfg.Worker.Password = password
	}

	// 更新 probe.target.url
	if probeURL, ok := req["probe.target.url"].(string); ok && probeURL != "" {
		s.cfg.Probe.Target.URL = probeURL
	}

	// 更新 probe.keep
	if keep, ok := req["probe.keep"].(float64); ok && keep > 0 {
		s.cfg.Probe.Keep = int(keep)
	}

	// 更新 probe.concurrency
	if concurrency, ok := req["probe.concurrency"].(float64); ok && concurrency > 0 {
		s.cfg.Probe.Concurrency = int(concurrency)
	}

	// 更新 probe.timeout_ms
	if timeout, ok := req["probe.timeout_ms"].(float64); ok && timeout > 0 {
		s.cfg.Probe.TimeoutMS = int(timeout)
	}

	// 更新 clash.local_profile_dir
	if clashDir, ok := req["clash.local_profile_dir"].(string); ok {
		s.cfg.Clash.LocalProfileDir = clashDir
	}

	// 更新 clash.host
	if clashHost, ok := req["clash.host"].(string); ok {
		s.cfg.Clash.Host = clashHost
	}

	// 更新 clash.uuid
	if clashUUID, ok := req["clash.uuid"].(string); ok {
		s.cfg.Clash.UUID = clashUUID
	}
